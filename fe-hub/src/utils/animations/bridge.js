/* eslint-disable */
/* Bridge: original animation engine with React null-safety patches */
try {
...
const btn=document.getElementById('br-btn'); if(btn){btn.textContent=BR.playing?'⏸ Pause':'▶ Play'; btn.classList.toggle('on',BR.playing);}
...
function brReset(){BR.step=0;BR.timer=0;BR.playing=true;(function(){const _e=document.getElementById('br-btn');if(_e)_e.textContent='⏸ Pause';})();document.getElementById('br-btn').classList.add('on');}
...
const btn=document.getElementById('eng-btn'); if(btn){btn.textContent=ENG.playing?'⏸ Pause':'▶ Play'; btn.classList.toggle('on',ENG.playing);}
...
(Array.from(document.querySelectorAll('[id^="subtab-"]'))||[]).forEach(el=>el.style.display='none'); const el=document.getElementById('subtab-'+name); if(el)el.style.display='block'; (Array.from(document.querySelectorAll('.tab-btn'))||[]).forEach(b=>b.classList.remove('active')); if(btn)btn.classList.add('active');
}
...
(Array.from(document.querySelectorAll('.pipeline-step'))||[]).forEach((s,j)=>s.classList.toggle('selected',i===j)); const d=document.getElementById('pipeline-detail');
...
function hlEngine(i){
(Array.from(document.querySelectorAll('.engine-step'))||[]).forEach((s,j)=>s.classList.toggle('hl',i===j)); const d=document.getElementById('engine-detail'); if(i>=0){d.style.display='block';d.textContent=engineDetails[i];} else d.style.display='none';
}
let mwStep=0;
...
(function(){const _e=document.getElementById('mw-status');if(_e)_e.textContent='';})();
mwStep=0;
...
(Array.from(document.querySelectorAll('#hca-css .tab-btn'))||[]).forEach(b => b.classList.remove('active'));
...
if(!document.getElementById('panel-react'))return;
// In React app: delegate to Redux dispatch via window.__reactSetTab
if(typeof window.__reactSetTab === 'function') {
  window.__reactSetTab(tab);
  return;
}
(Array.from(document.querySelectorAll('.tab-panel'))||[]).forEach(p=>p.classList.remove('active')); (Array.from(document.querySelectorAll('.header-nav button'))||[]).forEach(b=>b.classList.remove('active')); const panelEl=document.getElementById('panel-'+tab); if(panelEl) panelEl.classList.add('active'); const navBtn=document.getElementById('main-'+tab); if(navBtn) navBtn.classList.add('active');
currentTab=tab;
renderSidebar(tab);
...
if(id.startsWith('qa-filter-')){
  ...
  setTimeout(()=>{
    renderQA(cat);
    (Array.from(document.querySelectorAll('#qa-filter-bar button'))||[]).forEach(b=>{
      b.classList.toggle('active', b.getAttribute('onclick') && b.getAttribute('onclick').includes("'"+cat+"'"));
    });
  },50);
  (Array.from(document.querySelectorAll('.sidebar-item'))||[]).forEach(b=>b.classList.remove('active'));
  if(btn)btn.classList.add('active');
  return;
}
document.getElementById(id)?.scrollIntoView({behavior:'smooth',block:'start'}); (Array.from(document.querySelectorAll('.sidebar-item'))||[]).forEach(b=>b.classList.remove('active')); if(btn)btn.classList.add('active');
}
// Removed: React's useEffect handles init after DOM mount
...
(Array.from(document.querySelectorAll('.qa-card.open'))||[]).forEach(c => c&&c.classList&&c.classList.remove('open'));
if(!isOpen) card.classList.add('open');
}

function qaFilter(cat, btn){
(Array.from(document.querySelectorAll('#qa-filter-bar button'))||[]).forEach(b=>b.classList.remove('active')); if(btn) btn.classList.add('active');
renderQA(cat);
}

function beSelect(tech, btn){
(Array.from(document.querySelectorAll('.be-tech-card'))||[]).forEach(c=>c.classList.remove('active')); (Array.from(document.querySelectorAll('.be-section'))||[]).forEach(s=>s.classList.remove('active')); if(btn) btn.classList.add('active');
...
(Array.from(document.querySelectorAll('.hero'))||[]).forEach(hero => {
...
(Array.from(document.querySelectorAll('.hero'))||[]).forEach(hero => { if (!hero.querySelector('.hero-wm')) { hero.insertAdjacentHTML('beforeend', buildWM());
}
});
});
};
...
(Array.from(document.querySelectorAll('#panel-cicd .be-tech-card'))||[]).forEach(c=>c.classList.remove('active')); (Array.from(document.querySelectorAll('.cd-section'))||[]).forEach(s=>s.classList.remove('active')); if(btn) btn.classList.add('active');
...
(Array.from(document.querySelectorAll('#r19-animation .cbtn'))||[]).forEach(b => b.classList.remove('active'));
...
(Array.from(document.querySelectorAll('#r19-actions .cbtn'))||[]).forEach(b=>b.classList.remove('active'));
...
(Array.from(document.querySelectorAll('#r19-compiler .cbtn'))||[]).forEach(b=>b.classList.remove('active'));
...
(Array.from(document.querySelectorAll('#engine-wsw .cbtn'))||[]).forEach(b => b.classList.remove('active'));
if (btn) btn.classList.add('active');
if (!WSW.ctx) initWsw();
}
...
(Array.from(document.querySelectorAll('#hca-flexbox .cbtn'))||[]).forEach(b=>b.classList.remove('active'));
if(btn)btn.classList.add('active');
...
(Array.from(document.querySelectorAll('#hca-grid .cbtn'))||[]).forEach(b=>b.classList.remove('active'));
if(btn)btn.classList.add('active');
...
(function(){const _e=document.getElementById('search-clear');if(_e)_e.classList.toggle('visible',q.length>0);})();
...
(Array.from(document.querySelectorAll('.sidebar-item'))||[]).forEach(b =>
...
input.value = '';
(function(){const _e=document.getElementById('search-clear');if(_e)_e.classList.remove('visible');})();
closeDropdown();
input.blur();
};
...
activeIdx = idx;
(Array.from(document.querySelectorAll('.search-result-item'))||[]).forEach(btn => {
const on = parseInt(btn.dataset.idx) === idx;
if(btn&&btn.classList) btn.classList.toggle('active-result', on);
...
function openDropdown() {
  positionDropdown();
  const _e = document.getElementById('search-dropdown');
  if (!_e) return;
  _e.style.display = ''; // clear inline display:none so CSS .open class takes effect
  _e.classList.add('open');
  // Ensure delegated click handler exists (idempotent)
  if (!_e._delegated) {
    _e._delegated = true;
    _e.addEventListener('mousedown', function(ev) {
      const btn = ev.target.closest('[data-search-idx]');
      if (btn) {
        ev.preventDefault();
        const idx = parseInt(btn.getAttribute('data-search-idx'));
        if (!isNaN(idx)) handleSearchSelect(idx);
      }
    });
  }
}

function closeDropdown() { const _e=document.getElementById('search-dropdown'); if(_e){ _e.classList.remove('open'); _e.style.display='none'; } activeIdx = -1; }
...
(Array.from(document.querySelectorAll('#panel-sdlc .canvas-controls .cbtn'))||[]).forEach(b=>b.classList.remove('on'));
...
(Array.from(document.querySelectorAll('#cvs-architect~.canvas-controls .cbtn'))||[]).forEach(b=>b.classList.remove('on'));
if(btn)btn.classList.add('on');
...
if (typeof window !== 'undefined') {
  window._feHubAnimationsLoaded = true;
  window.switchMain = typeof switchMain !== 'undefined' ? switchMain : null;
  window.renderSidebar = typeof renderSidebar !== 'undefined' ? renderSidebar : null;
  window.scrollToSection = typeof scrollToSection !== 'undefined' ? scrollToSection : null;
  window.switchSubTab = typeof switchSubTab !== 'undefined' ? switchSubTab : null;
  window.renderQA = typeof renderQA !== 'undefined' ? renderQA : null;
  window.qaFilter = typeof qaFilter !== 'undefined' ? qaFilter : null;
  window.toggleQA = typeof toggleQA !== 'undefined' ? toggleQA : null;
  window.toggleTheme = typeof toggleTheme !== 'undefined' ? toggleTheme : null;
  window.clearSearch = typeof clearSearch !== 'undefined' ? clearSearch : null;
  window.handleSearchInput = typeof handleSearchInput !== 'undefined' ? handleSearchInput : null;
  window.handleSearchKeydown = typeof handleSearchKeydown !== 'undefined' ? handleSearchKeydown : null;
  window.handleSearchFocus = typeof handleSearchFocus !== 'undefined' ? handleSearchFocus : null;
  window.handleSearchBlur = typeof handleSearchBlur !== 'undefined' ? handleSearchBlur : null;
  window.beSelect = typeof beSelect !== 'undefined' ? beSelect : null;
  window.cdSelect = typeof cdSelect !== 'undefined' ? cdSelect : null;
  window.hcaCssTab = typeof hcaCssTab !== 'undefined' ? hcaCssTab : null;
  window.selectPipeline = typeof selectPipeline !== 'undefined' ? selectPipeline : null;
  window.hlEngine = typeof hlEngine !== 'undefined' ? hlEngine : null;
  window.wlcGo = typeof wlcGo !== 'undefined' ? wlcGo : null;
  window.sdlcMode = typeof sdlcMode !== 'undefined' ? sdlcMode : null;
  window.archMode = typeof archMode !== 'undefined' ? archMode : null;
  window.flexMode = typeof flexMode !== 'undefined' ? flexMode : null;
  window.gridMode = typeof gridMode !== 'undefined' ? gridMode : null;
  window.wswMode = typeof wswMode !== 'undefined' ? wswMode : null;
  window.r19ActAnim = typeof r19ActAnim !== 'undefined' ? r19ActAnim : null;
  window.r19CmpAnim = typeof r19CmpAnim !== 'undefined' ? r19CmpAnim : null;
  window.initAngular = typeof initAngular !== 'undefined' ? initAngular : null;
  window.initArchitectAnim = typeof initArchitectAnim !== 'undefined' ? initArchitectAnim : null;
  window.initAsync = typeof initAsync !== 'undefined' ? initAsync : null;
  window.initBeStack = typeof initBeStack !== 'undefined' ? initBeStack : null;
  window.initBoxModel = typeof initBoxModel !== 'undefined' ? initBoxModel : null;
  window.initBrowser = typeof initBrowser !== 'undefined' ? initBrowser : null;
  window.initBuildCmp = typeof initBuildCmp !== 'undefined' ? initBuildCmp : null;
  window.initCicdPipeline = typeof initCicdPipeline !== 'undefined' ? initCicdPipeline : null;
  window.initFiber = typeof initFiber !== 'undefined' ? initFiber : null;
  window.initHoc = typeof initHoc !== 'undefined' ? initHoc : null;
  window.initContext = typeof initContext !== 'undefined' ? initContext : null;
  window.initDockerAnim = typeof initDockerAnim !== 'undefined' ? initDockerAnim : null;
  window.initEngine = typeof initEngine !== 'undefined' ? initEngine : null;
  window.initEventLoop = typeof initEventLoop !== 'undefined' ? initEventLoop : null;
  window.initExpressArch = typeof initExpressArch !== 'undefined' ? initExpressArch : null;
  window.initExpressMw = typeof initExpressMw !== 'undefined' ? initExpressMw : null;
  window.initFlexbox = typeof initFlexbox !== 'undefined' ? initFlexbox : null;
  window.initFragment = typeof initFragment !== 'undefined' ? initFragment : null;
  window.initGraphql = typeof initGraphql !== 'undefined' ? initGraphql : null;
  window.initGrid = typeof initGrid !== 'undefined' ? initGrid : null;
  window.initK8sAnim = typeof initK8sAnim !== 'undefined' ? initK8sAnim : null;
  window.initKafkaAnim = typeof initKafkaAnim !== 'undefined' ? initKafkaAnim : null;
  window.initMongoArch = typeof initMongoArch !== 'undefined' ? initMongoArch : null;
  window.initMonorepo = typeof initMonorepo !== 'undefined' ? initMonorepo : null;
  window.initCsr = typeof initCsr !== 'undefined' ? initCsr : null;
  window.initSsr = typeof initSsr !== 'undefined' ? initSsr : null;
  window.initNextflow = typeof initNextflow !== 'undefined' ? initNextflow : null;
  window.initNodeEventLoop = typeof initNodeEventLoop !== 'undefined' ? initNodeEventLoop : null;
  window.initNodeStreams = typeof initNodeStreams !== 'undefined' ? initNodeStreams : null;
  window.initPat = typeof initPat !== 'undefined' ? initPat : null;
  window.initPatModule = typeof initPatModule !== 'undefined' ? initPatModule : null;
  window.initPatSingleton = typeof initPatSingleton !== 'undefined' ? initPatSingleton : null;
  window.initPatFactory = typeof initPatFactory !== 'undefined' ? initPatFactory : null;
  window.initPatObserver = typeof initPatObserver !== 'undefined' ? initPatObserver : null;
  window.initPatStrategy = typeof initPatStrategy !== 'undefined' ? initPatStrategy : null;
  window.initPgArch = typeof initPgArch !== 'undefined' ? initPgArch : null;
  window.initPgNode = typeof initPgNode !== 'undefined' ? initPgNode : null;
  window.initPortal = typeof initPortal !== 'undefined' ? initPortal : null;
  window.initPreprocessors = typeof initPreprocessors !== 'undefined' ? initPreprocessors : null;
  window.initR19Actions = typeof initR19Actions !== 'undefined' ? initR19Actions : null;
  window.initR19Compiler = typeof initR19Compiler !== 'undefined' ? initR19Compiler : null;
  window.initReact = typeof initReact !== 'undefined' ? initReact : null;
  window.initReact19Anim = typeof initReact19Anim !== 'undefined' ? initReact19Anim : null;
  window.initScanning = typeof initScanning !== 'undefined' ? initScanning : null;
  window.initSdlcAnim = typeof initSdlcAnim !== 'undefined' ? initSdlcAnim : null;
  window.initSecurity = typeof initSecurity !== 'undefined' ? initSecurity : null;
  window.initTestPattern = typeof initTestPattern !== 'undefined' ? initTestPattern : null;
  window.initTesting = typeof initTesting !== 'undefined' ? initTesting : null;
  window.initVercelAnim = typeof initVercelAnim !== 'undefined' ? initVercelAnim : null;
  window.initVitals = typeof initVitals !== 'undefined' ? initVitals : null;
  window.initVitalsTimeline = typeof initVitalsTimeline !== 'undefined' ? initVitalsTimeline : null;
  window.initVue = typeof initVue !== 'undefined' ? initVue : null;
  window.initVite = typeof initVite !== 'undefined' ? initVite : null;
  window.initWebpack = typeof initWebpack !== 'undefined' ? initWebpack : null;
  window.initWebpackAdv = typeof initWebpackAdv !== 'undefined' ? initWebpackAdv : null;
  window.initWsw = typeof initWsw !== 'undefined' ? initWsw : null;
  window.cicdPipelineAnim = typeof cicdPipelineAnim !== 'undefined' ? cicdPipelineAnim : null;
  window.angularAnim = typeof angularAnim !== 'undefined' ? angularAnim : null;
  window.asyncRun = typeof asyncRun !== 'undefined' ? asyncRun : null;
  window.beStackAnim = typeof beStackAnim !== 'undefined' ? beStackAnim : null;
  window.brNext = typeof brNext !== 'undefined' ? brNext : null;
  window.brPrev = typeof brPrev !== 'undefined' ? brPrev : null;
  window.brReset = typeof brReset !== 'undefined' ? brReset : null;
  window.buildCmpAnim = typeof buildCmpAnim !== 'undefined' ? buildCmpAnim : null;
  window.contextAnim = typeof contextAnim !== 'undefined' ? contextAnim : null;
  window.csrAnim = typeof csrAnim !== 'undefined' ? csrAnim : null;
  window.dockerAnim = typeof dockerAnim !== 'undefined' ? dockerAnim : null;
  window.elRun = typeof elRun !== 'undefined' ? elRun : null;
  window.engCode = typeof engCode !== 'undefined' ? engCode : null;
  window.expressAnim = typeof expressAnim !== 'undefined' ? expressAnim : null;
  window.expressArchAnim = typeof expressArchAnim !== 'undefined' ? expressArchAnim : null;
  window.fiberAnim = typeof fiberAnim !== 'undefined' ? fiberAnim : null;
  window.fragmentAnim = typeof fragmentAnim !== 'undefined' ? fragmentAnim : null;
  window.gqlAnim = typeof gqlAnim !== 'undefined' ? gqlAnim : null;
  window.hocAnim = typeof hocAnim !== 'undefined' ? hocAnim : null;
  window.k8sAnim = typeof k8sAnim !== 'undefined' ? k8sAnim : null;
  window.kafkaAnim = typeof kafkaAnim !== 'undefined' ? kafkaAnim : null;
  window.mongoAnim = typeof mongoAnim !== 'undefined' ? mongoAnim : null;
  window.monorepoAnim = typeof monorepoAnim !== 'undefined' ? monorepoAnim : null;
  window.nextflowAnim = typeof nextflowAnim !== 'undefined' ? nextflowAnim : null;
  window.nodeAnim = typeof nodeAnim !== 'undefined' ? nodeAnim : null;
  window.nodeStreamAnim = typeof nodeStreamAnim !== 'undefined' ? nodeStreamAnim : null;
  window.patAnim = typeof patAnim !== 'undefined' ? patAnim : null;
  window.pgAnim = typeof pgAnim !== 'undefined' ? pgAnim : null;
  window.pgNodeAnim = typeof pgNodeAnim !== 'undefined' ? pgNodeAnim : null;
  window.portalAnim = typeof portalAnim !== 'undefined' ? portalAnim : null;
  window.r19Anim = typeof r19Anim !== 'undefined' ? r19Anim : null;
  window.reactAnim = typeof reactAnim !== 'undefined' ? reactAnim : null;
  window.scanAnim = typeof scanAnim !== 'undefined' ? scanAnim : null;
  window.secAnim = typeof secAnim !== 'undefined' ? secAnim : null;
  window.ssrAnim = typeof ssrAnim !== 'undefined' ? ssrAnim : null;
  window.testPatAnim = typeof testPatAnim !== 'undefined' ? testPatAnim : null;
  window.testingAnim = typeof testingAnim !== 'undefined' ? testingAnim : null;
  window.vercelAnim = typeof vercelAnim !== 'undefined' ? vercelAnim : null;
  window.vitalsAnim = typeof vitalsAnim !== 'undefined' ? vitalsAnim : null;
  window.vitalsTimelineAnim = typeof vitalsTimelineAnim !== 'undefined' ? vitalsTimelineAnim : null;
  window.viteAnim = typeof viteAnim !== 'undefined' ? viteAnim : null;
  window.vueAnim = typeof vueAnim !== 'undefined' ? vueAnim : null;
  window.webpackAdvAnim = typeof webpackAdvAnim !== 'undefined' ? webpackAdvAnim : null;
  window.webpackAnim = typeof webpackAnim !== 'undefined' ? webpackAnim : null;
  // Hero watermark injection - called after React renders
  window.injectHeroWatermarks = function() {
    const crossSVG = `<svg viewBox="0 0 24 24" fill="none"><line x1="12" y1="2" x2="12" y2="22" stroke="white" stroke-width="1"/><line x1="2" y1="12" x2="22" y2="12" stroke="white" stroke-width="1"/></svg>`;
    const hexSVG = `<svg viewBox="0 0 32 32" fill="none"><polygon points="16,2 28,9 28,23 16,30 4,23 4,9" stroke="white" stroke-width="1"/></svg>`;
    const circSVG = `<svg viewBox="0 0 32 32" fill="none"><circle cx="16" cy="16" r="12" stroke="white" stroke-width="1"/><circle cx="16" cy="16" r="6" stroke="white" stroke-width="0.7"/></svg>`;
    const wm = `<div class="hero-wm" aria-hidden="true">
<div class="wm-ring" style="left:68%;top:10%;width:160px;height:160px;margin:-80px 0 0 -80px;animation-delay:0s;animation-duration:5s;"></div>
<div class="wm-ring" style="left:82%;top:55%;width:100px;height:100px;margin:-50px 0 0 -50px;animation-delay:1.5s;animation-duration:4s;"></div>
<div class="wm-ring2" style="left:55%;top:-5%;width:220px;height:220px;margin:-110px 0 0 -110px;animation-delay:0.8s;animation-duration:7s;"></div>
<div class="wm-ring2" style="left:90%;top:80%;width:140px;height:140px;margin:-70px 0 0 -70px;animation-delay:3s;animation-duration:6s;"></div>
...
<div class="wm-line" style="left:55%;top:70%;animation-delay:3s;animation-duration:10s;opacity:0.7;"></div>
    ...
    (Array.from(document.querySelectorAll('.hero'))||[]).forEach(function(hero) {
      if (!hero.querySelector('.hero-wm')) {
        hero.insertAdjacentHTML('beforeend', wm);
      }
    });
  };
}
...
} catch(e) { console.warn("[Bridge] Init error:", e.message, e.stack); }
